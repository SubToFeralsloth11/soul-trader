...
public class WhisperEntity extends PassiveEntity {
    ...
    private static final List<ItemStack> SELL_REWARDS = List.of(
    ...
    public WhisperEntity(EntityType<? extends PassiveEntity> entityType, World world) {
        super(entityType, world);
        this.setPersistent();
        if (!world.isClient) {
            this.dataTracker.set(COST_TYPE, this.random.nextInt(3));
        }
    }
    ...
    protected void initGoals() {
        ...
        this.goalSelector.add(1, new FleeEntityGoal<>(this, PlayerEntity.class, 10.0f, 1.3, 1.6));
    ...
    public ItemStack getSellReward() {
        if (this.sellReward.isEmpty()) {
            regenerateSellReward();
        }
    ...
    private void regenerateSellReward() {
        if (this.getWorld().isClient) return;
        this.sellReward = SELL_REWARDS.get(this.random.nextInt(SELL_REWARDS.size())).copy();
    }
